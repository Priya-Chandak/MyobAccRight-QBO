...
def get_qbo_spend_overpayment(job_id,task_id):
    try:
        myclient = MongoClient("mongodb://localhost:27017/")
        db1 = myclient["MMC"]
        QBO_SPEND_OVERPAYMENT1 = db1['QBO_SPEND_OVERPAYMENT']
        base_url, headers, company_id, minorversion, get_data_header,report_headers = get_settings_qbo(job_id)

        url = f"{base_url}/query?minorversion={minorversion}"
        no_of_records = db1['QBO_SPEND_OVERPAYMENT'].count_documents({'job_id':job_id})
        ...
        QBO_SPEND_OVERPAYMENT1.insert_many(archivedcoa)

        if JsonResponse['QueryResponse']['maxResults'] < 1000:
            Break
        else:
            get_qbo_spend_overpayment(job_id)
